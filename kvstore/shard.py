...
class Shard:
        ...
        for i in range(replica_count):
            try:
                ...
                while True:
                    ...
                    try:
                        print(nodes[i].init(raft_dir=raft_dir_path+suffix, raft_port=raft_ports[i], store_dir=store_dir_path+suffix, node_id=i, leader=(i == self.leader_id)), f"at node_id {i}")
                        # print(nodes[i].init(raft_dir=raft_dir_path+suffix, raft_port=raft_ports[i], store_dir=store_dir_path+suffix, node_id=i, clear_store_dir= not ((i == self.leader_id) and found_last_log)), f"at node_id {i}")
                        break
                    except:
                        pass
    ...
    def update_leader(self, dont_check_failures: bool = False):
        ...
        if dont_check_failures:
            print("WARN: Reached more failures than can handle but don't check more failures was passed!")
            return
        ...
        for id in inactive_ones:
            self.nodes[id].force_kill()
        sleep(self.wait_time)
        for id in inactive_ones:
            self.nodes[id].force_init()
        sleep(self.wait_time)
        self.update_leader()
    ...
    def get(self, key: str, already_tried: set = set()):
        ...
        if len(to_try) == 0:
            print("ERROR: Wasn't able to reach any node through Grpc, check server side printed log to know all nodes offline")
            return -1
